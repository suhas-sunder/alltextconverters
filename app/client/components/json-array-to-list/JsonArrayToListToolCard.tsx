...
import {
  jsonArrayToList,
  type JsonArrayToListOptions,
} from "./jsonArrayToListTransforms";

type JsonArrayToListToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function JsonArrayToListToolCard({
  input,
  setInput,
}: JsonArrayToListToolCardProps) {
  const [copied, setCopied] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [infoMsg, setInfoMsg] = useState<string | null>(null);

  const [ignoreEmpty, setIgnoreEmpty] = useState(true);
  const [stringifyObjects, setStringifyObjects] = useState(true);
  const [trimValues, setTrimValues] = useState(true);

  const options: JsonArrayToListOptions = useMemo(
    () => ({ ignoreEmpty, stringifyObjects, trimValues }),
    [ignoreEmpty, stringifyObjects, trimValues],
  );
  ...
  function convert() {
    ...
    const res = jsonArrayToList(input, options);
    if (!res.ok) {
    ...
    msgParts.push(
      `Extracted ${res.stats.linesOut} line${res.stats.linesOut === 1 ? "" : "s"} from ${res.stats.itemsInArray} array item${res.stats.itemsInArray === 1 ? "" : "s"}.`,
    );
    if (res.stats.ignoredEmpty) {
      msgParts.push(
        `Ignored ${res.stats.ignoredEmpty} empty value${res.stats.ignoredEmpty === 1 ? "" : "s"}.`,
      );
    }
    if (res.stats.stringifiedComplex && stringifyObjects) {
      msgParts.push(
        `Stringified ${res.stats.stringifiedComplex} non-primitive value${res.stats.stringifiedComplex === 1 ? "" : "s"}.`,
      );
    }
    setInfoMsg(msgParts.join(" "));
  }
  ...
  const downloadAsPdf = async () => {
    try {
      ...
      doc.save("json-array-to-list.pdf");
    } catch {
      window.print();
    }
  };
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-6 sm:px-8 pb-8 pt-4 space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        <textarea
          ...
          placeholder='Paste a JSON array and convert it into a clean, line-by-line list. For example: ["apple", "banana", "orange"]'
        ...
        <div className="mt-4 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                aria-label="Convert JSON array to list"
              >
                Convert to list
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            />
            <Toggle
              label="Stringify objects and arrays"
              checked={stringifyObjects}
              onChange={setStringifyObjects}
            />
          </div>
        </div>
      </div>
...
function Toggle({
  ...
  return (
    <button
      ...
      className="cursor-pointer flex items-center justify-between gap-3 rounded-2xl bg-white ring-1 ring-slate-200/80 px-4 py-3 hover:ring-sky-200/80 transition"
    ...
    >
      ...
      <span
        className={
          checked
            ? "inline-flex items-center rounded-full bg-sky-50 text-sky-700 ring-1 ring-sky-200/70 px-3 py-1 text-xs font-semibold"
            : "inline-flex items-center rounded-full bg-slate-50 text-slate-700 ring-1 ring-slate-200 px-3 py-1 text-xs font-semibold"
        }
      >
        {checked ? "On" : "Off"}
      </span>
