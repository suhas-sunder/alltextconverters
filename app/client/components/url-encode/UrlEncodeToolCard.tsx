...
import { toUrlEncoded } from "./urlEncodeTransforms";

type UrlEncodeToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function UrlEncodeToolCard({ input, setInput }: UrlEncodeToolCardProps) {
  ...
  function encodeNow() {
    ...
    try {
      const out = toUrlEncoded(input);
      setInput(out);
    } catch (e) {
      console.error(e);
      setErrorMsg(
        "Encoding failed. This can happen if your text contains an invalid Unicode character sequence. Try removing unusual invisible characters and retry.",
      );
    }
  }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      setInput(toUrlEncoded(text));
      setInfoMsg("File loaded locally.");
    } catch (e) {
      console.error(e);
      setErrorMsg(
        "Could not extract and encode text from that file in-browser. Make sure optional libraries are installed (pdfjs-dist for PDF, mammoth for DOCX).",
      );
    }
  };
  ...
  const downloadAsText = () => {
    try {
      ...
      a.href = url;
      a.download = "url-encoded-text.txt";
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                aria-label="Encode text for URL"
              >
                Encode URL
              </button>
            </div>
