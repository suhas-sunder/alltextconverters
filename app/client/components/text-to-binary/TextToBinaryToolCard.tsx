...
import { textToBinary, type BitWidth } from "./textToBinaryTransforms";

type TextToBinaryToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function TextToBinaryToolCard({
  input,
  setInput,
}: TextToBinaryToolCardProps) {
  ...
  const statsLine = useMemo(() => {
    ...
    return chars ? `${chars.toLocaleString()} chars` : "Empty";
  }, [input]);
  ...
  function convert() {
    ...
    const res = textToBinary(input, bitWidth);
    if (!res.ok) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        setInput(cleaned);
        setInfoMsg("PDF text extracted locally. Ready to convert.");
        return;
      }
      ...
      if (ext === "docx") {
        ...
        setInput(value);
        setInfoMsg("DOCX text extracted locally. Ready to convert.");
        return;
      }

      if (ext === "bin") {
        // Raw bytes -> 8-bit binary groups (ASCII is a subset; still warn about non-ASCII bytes).
        const bytes = new Uint8Array(await file.arrayBuffer());
        if (!bytes.length) throw new Error("Empty .bin file");

        const groups = Array.from(bytes, (b) => b.toString(2).padStart(8, "0"));
        setInput(groups.join(" "));
        setInfoMsg(
          "Loaded .bin locally as 8-bit binary groups. If you want text, use Binary to Text instead.",
        );
        return;
      }
      ...
      setInput(text);
      setInfoMsg("File loaded locally. Ready to convert.");
    } catch (e) {
      console.error(e);
      setErrorMsg(
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="inline-flex items-center rounded-full bg-white ring-1 ring-slate-200 shadow-sm overflow-hidden">
              {([7, 8] as const).map((n) => {
                const active = bitWidth === n;
                return (
                  <button
                    key={n}
                    type="button"
                    onClick={() => setBitWidth(n)}
                    className={
        ...
        <textarea
          ...
          placeholder="Paste or type ASCII text here…"
