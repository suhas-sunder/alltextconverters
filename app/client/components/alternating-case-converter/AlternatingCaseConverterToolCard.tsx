...
import { toAlternatingCase } from "./alternatingCaseTransforms";

type AlternatingCaseConverterToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function AlternatingCaseConverterToolCard({
  input,
  setInput,
}: AlternatingCaseConverterToolCardProps) {
  ...
  function convertToAlternatingCase() {
    setErrorMsg(null);
    setInfoMsg(null);
    setInput(toAlternatingCase(input, ignoreSpaces));
  }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        setInput(toAlternatingCase(cleaned, ignoreSpaces));
      ...
      if (ext === "docx") {
        ...
        setInput(toAlternatingCase(value, ignoreSpaces));
        setInfoMsg(
      ...
      setInput(toAlternatingCase(text, ignoreSpaces));
  ...
  const downloadAsPdf = async () => {
    try {
      ...
      doc.save("alternating-case-text.pdf");
    } catch {
          ...
          <div className="flex items-center gap-2 text-xs text-slate-600">
            <label className="cursor-pointer inline-flex items-center gap-2">
              <input
                ...
                className="cursor-pointer h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-300"
              />
              Ignore spaces when alternating
            </label>
            <span className="text-slate-500">(punctuation is always ignored)</span>
          </div>
        </div>
      </div>
    </div>
