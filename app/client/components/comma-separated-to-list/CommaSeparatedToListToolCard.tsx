...
import { commaSeparatedToList } from "./commaSeparatedToListTransforms";
...
export function CommaSeparatedToListToolCard({ input, setInput }: Props) {
  ...
  function convert() {
    ...
    const { items, output } = commaSeparatedToList(input);
    setInput(output);

    if (items.length === 0) {
      setInfoMsg("No values found. Add comma-separated items, then convert.");
      return;
    }
  }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        setInput(cleaned);
        setInfoMsg("PDF text extracted locally. Convert when ready.");
        return;
      }
      ...
      if (ext === "docx") {
        ...
        setInput(value);
        setInfoMsg("DOCX text extracted locally. Convert when ready.");
        return;
      }
      ...
      setInput(text);
      setInfoMsg("File loaded locally. Convert when ready.");
    } catch (e) {
      console.error(e);
      setErrorMsg(
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-6 sm:px-8 pb-8 pt-4 space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        <textarea
          ...
          placeholder="Paste values like a, b, c and convert them into a clean newline list. Example: apple, banana, cherry"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                aria-label="Convert comma-separated values to list"
              >
                Convert to List
              </button>
            </div>
