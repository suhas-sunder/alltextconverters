...
import { toSentenceCase } from "./sentenceCaseTransforms";

type SentenceCaseConverterToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function SentenceCaseConverterToolCard({
  input,
  setInput,
}: SentenceCaseConverterToolCardProps) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        setInput(toSentenceCase(cleaned));
  ...
  const downloadAsPdf = async () => {
    try {
      ...
      doc.save("sentence-case-text.pdf");
    } catch {
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="text-xs text-slate-500">
            Capitalizes after <span className="font-semibold text-slate-700">.</span>{" "}
            <span className="font-semibold text-slate-700">!</span>{" "}
            <span className="font-semibold text-slate-700">?</span> using simple rules. No grammar fixes.
          </div>
        </div>
      </div>
    </div>
