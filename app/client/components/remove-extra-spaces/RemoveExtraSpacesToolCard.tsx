...
import { removeExtraSpaces } from "./removeExtraSpacesTransforms";

type RemoveExtraSpacesToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function RemoveExtraSpacesToolCard({
  input,
  setInput,
}: RemoveExtraSpacesToolCardProps) {
  ...
  function cleanSpaces() {
    ...
    const res = removeExtraSpaces(input, { preserveLineBreaks });
    setInput(res.output);

    const parts: string[] = [];
    parts.push("Collapsed extra spaces");
    if (preserveLineBreaks) parts.push("kept line breaks");
    else parts.push("collapsed line breaks");
    if (res.stats.removedZeroWidth > 0) parts.push("removed invisible characters");
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        const res = removeExtraSpaces(extracted, { preserveLineBreaks });
        setInput(res.output);
        setInfoMsg("PDF text extracted locally. Spacing cleaned.");
        return;
      }
      ...
      if (ext === "docx") {
        ...
        const res = removeExtraSpaces(value, { preserveLineBreaks });
        setInput(res.output);
        setInfoMsg("DOCX text extracted locally. Spacing cleaned.");
        return;
      }

      const text = await readAsText();
      const res = removeExtraSpaces(text, { preserveLineBreaks });
      setInput(res.output);
      setInfoMsg("File loaded locally. Spacing cleaned.");
    } catch (e) {
      console.error(e);
      setErrorMsg(
  ...
  const downloadAsPdf = async () => {
    try {
      ...
      doc.save("remove-extra-spaces.pdf");
    } catch {
      window.print();
    }
  };
