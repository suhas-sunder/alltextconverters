...
import { decodeUrlEncodedText } from "./urlDecodeTransforms";

type UrlDecodeToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function UrlDecodeToolCard({ input, setInput }: UrlDecodeToolCardProps) {
  ...
  function runDecode() {
    ...
    const result = decodeUrlEncodedText(input);
    if (!result.ok) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        setInput(cleaned);
        setInfoMsg("PDF text extracted locally. Click Decode to process.");
        return;
      }
      ...
      if (ext === "docx") {
        ...
        setInput(value);
        setInfoMsg("DOCX text extracted locally. Click Decode to process.");
        return;
      }

      const text = await readAsText();
      setInput(text);
      setInfoMsg("File loaded locally. Click Decode to process.");
    } catch (e) {
      console.error(e);
      setErrorMsg(
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-6 sm:px-8 pb-8 pt-4 space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        <textarea
          ...
          placeholder="Paste URL-encoded text here… (example: hello%20world%21)"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                aria-label="Decode URL-encoded text"
              >
                Decode
              </button>
            </div>
