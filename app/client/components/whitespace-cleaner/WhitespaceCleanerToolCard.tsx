...
import { cleanWhitespace } from "./whitespaceCleanerTransforms";

type WhitespaceCleanerToolCardProps = {
  input: string;
...
export function WhitespaceCleanerToolCard({
  input,
  setInput,
}: WhitespaceCleanerToolCardProps) {
  const [copied, setCopied] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [infoMsg, setInfoMsg] = useState<string | null>(null);
  const [applied, setApplied] = useState<AppliedOp[]>([]);
  ...
  function runClean() {
    ...
    const res = cleanWhitespace(input);
    setInput(res.text);

    const ops: AppliedOp[] = [
      { label: "Tabs replaced with spaces", count: res.counts.tabsReplaced },
      { label: "Non-breaking spaces (NBSP) replaced", count: res.counts.nbspReplaced },
      { label: "Zero-width characters removed", count: res.counts.zeroWidthRemoved },
      { label: "Other invisible characters removed", count: res.counts.invisibleRemoved },
    ].filter((o) => o.count > 0);

    setApplied(ops);

    if (res.counts.totalChanged === 0) {
      setInfoMsg("No hidden whitespace or invisible characters were found.");
    } else {
      setInfoMsg(
        `Cleaned hidden whitespace. Removed or replaced ${res.counts.totalChanged} character${res.counts.totalChanged === 1 ? "" : "s"}.`,
      );
    }
  }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        setInput(extracted);
        setInfoMsg("PDF text extracted locally. Click Clean to remove hidden whitespace.");
        return;
      }
      ...
      if (ext === "docx") {
        ...
        setInput(value);
        setInfoMsg("DOCX text extracted locally. Click Clean to remove hidden whitespace.");
        return;
      }

      const text = await readAsText();
      setInput(text);
      setInfoMsg("File loaded locally. Click Clean to remove hidden whitespace.");
    } catch (e) {
      console.error(e);
      setErrorMsg(
  ...
  const downloadAsPdf = async () => {
    try {
      ...
      doc.save("whitespace-cleaned.pdf");
    } catch {
      window.print();
    }
  };
  ...
  const appliedSummary = useMemo(() => {
    const total = applied.reduce((sum, it) => sum + it.count, 0);
    return total;
  }, [applied]);
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-1 text-sm text-slate-600">
                  {appliedSummary === 0
                    ...
                    : "Summary of what was removed or replaced in the last run."}
                </div>
              </div>
              <div className="shrink-0">
                <span className="inline-flex items-center gap-2 rounded-full bg-white text-slate-700 ring-1 ring-slate-200 px-3 py-1 text-xs font-semibold">
                  ...
                  {appliedSummary} change{appliedSummary === 1 ? "" : "s"}
                </span>
              </div>
            </div>
                ...
                {applied.map((op) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-sm text-slate-700">
                      {op.count} character{op.count === 1 ? "" : "s"}
                    </div>
                  </li>
                ))}
              </ul>
