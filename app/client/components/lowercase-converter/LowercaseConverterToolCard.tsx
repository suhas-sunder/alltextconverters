...
type LowercaseConverterToolCardProps = {
  input: string;
  setInput: (v: string) => void;
};

export function LowercaseConverterToolCard({
  input,
  setInput,
}: LowercaseConverterToolCardProps) {
  ...
  const downloadAsPdf = async () => {
    try {
      ...
      doc.save("lowercase-converter-text.pdf");
    } catch {
      window.print();
    }
  };
  ...
  const primaryButton =
    "cursor-pointer inline-flex items-center rounded-full bg-sky-600 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-sky-700 transition";

  const disabledPrimaryButton =
    "inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-400 ring-1 ring-slate-200 shadow-sm cursor-not-allowed";

  const hasText = Boolean((input ?? "").length);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                disabled={!hasText}
                className={hasText ? primaryButton : disabledPrimaryButton}
