...
type CaseConverterToolCardProps = {
  input: string;
...
export function CaseConverterToolCard({
  input,
  setInput,
}: CaseConverterToolCardProps) {
  ...
  function handleModeChange(next: CaseMode) {
    setMode(next);
    // Convert existing content immediately when switching modes
    setInput(applyCaseTransform(input, next));
  }

  function handleChange(nextRaw: string) {
    // Enforce the selected mode as the user types/pastes
    setInput(applyCaseTransform(nextRaw, mode));
  }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      if (ext === "pdf") {
        ...
        setInput(applyCaseTransform(cleaned, mode));
      ...
      if (ext === "docx") {
        ...
        setInput(applyCaseTransform(value, mode));
        setInfoMsg(
      ...
      setInput(applyCaseTransform(text, mode));
  ...
  const downloadAsText = () => {
    try {
      ...
      a.href = url;
      a.download = "case-converter-text.txt";
  ...
  const downloadAsPdf = async () => {
    try {
      ...
      // Preserve newlines while still wrapping long lines.
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        // Add a blank line between paragraphs (but not after last)
        if (i < paragraphs.length - 1) y += 8;
      }

      doc.save("case-converter-text.pdf");
    } catch {
        ...
        {CASE_MODES.map((m) => {
          ...
          return (
            <button
              ...
              className={
                active
                  ? "cursor-pointer inline-flex items-center rounded-full bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-sky-700 transition"
                  : "cursor-pointer inline-flex items-center rounded-full bg-white px-4 py-2 text-sm font-semibold text-slate-800 ring-1 ring-slate-200 shadow-sm hover:ring-sky-200 hover:bg-sky-50 transition"
              }
            >
              {MODE_LABEL[m] ?? m}
            </button>
